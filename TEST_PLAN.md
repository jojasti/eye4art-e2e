...
| 19  | Submitting the contact form sends the expected request (mocked) | `EMAILJS_URL_PATTERN` (`api.emailjs.com`) is already aborted globally by the fixture, like analytics. This scenario overrides that one route with its own `page.route` handler (`fulfill` instead of `abort`) so the request can be inspected — asserts the intercepted request body and the success UI — **never a real submission, and no other test can accidentally send one either** | Verifies the only lead-generation form on the site actually attempts to send data, without ever risking spam to production | **critical** `@critical` |
